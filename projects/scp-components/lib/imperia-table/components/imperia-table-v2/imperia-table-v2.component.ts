...
import { ImperiaTableBlock } from '../../models/imperia-table-block.models';
...
} from '../../template-directives/imperia-table-body-cell-template.directive';
...
@Component({
  selector: 'imperia-table-v2',
  templateUrl: './imperia-table-v2.component.html',
  styleUrls: [
    './imperia-table-v2.caption.scss',
    './imperia-table-v2.loading.scss',
    './imperia-table-v2.block.scss',
    './imperia-table-v2.editing.scss',
    './imperia-table-v2.component.scss',
  ],
  animations: [
    HORIZONTAL_LIST_ELEMENT_ENTER_LEAVE,
    HORIZONTAL_ELEMENT_ENTER_LEAVE,
  ...
  implements ImperiaTableV2Host<TItem>
{
  ...
  //#region CELL OVERLAY PINNED LIST
  @ViewChild(ImperiaTableV2CellOverlayPinnedListComponent)
  set cellOverlayPinnedListComponentSetter(
    v: ImperiaTableV2CellOverlayPinnedListComponent<TItem> | undefined
  ) {
    if (!v) return;
    this.cellOverlayPinnedListComponent.next(v);
  }
  private cellOverlayPinnedListComponent = new ReplaySubject<
    ImperiaTableV2CellOverlayPinnedListComponent<TItem>
  >(1);
  public cellOverlayPinnedListComponent$ =
    this.cellOverlayPinnedListComponent.pipe(
      first(),
      shareReplay({ bufferSize: 1, refCount: true })
    );
  //#endregion CELL OVERLAY PINNED LIST
  ...
  /* storage$ emit his values from onFilterChanges at onFilterChange$ */
  public storage$ = this.storageKey.pipe(
    map((storageKey) => ({
      Order: getSortFromStorage<TItem>(storageKey),
      Filters: getFiltersFromStorage<TItem>(storageKey),
    })),
    switchMap(({ Order, Filters }) =>
      this.columnsConfigured$.pipe(
        take(1),
        map((columns) => {
          columns.forEach((col) => col.sort(Sort.NONE));
          const sortedColumn = columns.find(
            (col) => col.filterName == Order.Column && col.sortable
          );
          const OrderIsValid =
            sortedColumn && !!Order.Column && Order.Sort != Sort.NONE;
          if (OrderIsValid) {
            sortedColumn.sort(Order.Sort);
          }
          return {
            Order: OrderIsValid
              ? Order
              : new ImperiaTableSortValue<TItem>('', Sort.NONE),
            Filters: Filters.filter(({ Column }) =>
              columns.some((col) => col.filterName == Column)
            ),
            columns,
          };
        })
  ...
  //#region COLUMNS TEMPLATES FROM COLUMN DIRECTIVES
  private columnsTemplatesFromColumnsDirectives = defer(
    () => this.columnsDirectives
  ).pipe(
    first(),
    switchMap((columnsDirectives) =>
      columnsDirectives.changes.pipe(
        startWith(null),
        map(() =>
          columnsDirectives
            .toArray()
            .map((colDirective) => colDirective.bodyColumnTemplate$)
        )
      )
    )
  );

  private columnsTemplatesFromColumnsDirectives$: Observable<
    (
      | ImperiaTableBodyCellTemplateDirective<TItem>
      | ImpUnitNameComponent<TItem>
    )[]
  > = this.columnsTemplatesFromColumnsDirectives.pipe(
    switchMap((columnTemplates$) =>
      columnTemplates$.length
        ? combineLatest(columnTemplates$).pipe(
            map((templates) => templates.filter(isTruthy))
          )
        : of([])
    )
  );
  //#endregion COLUMNS TEMPLATES FROM COLUMN DIRECTIVES

  //#region COLUMNS BODY CELLS TEMPLATES
  ...
  private columnBodyCellTemplatesFromDirectivesChanges$: Observable<
    (
      | ImperiaTableBodyCellTemplateDirective<TItem>
      | ImpUnitNameComponent<TItem>
    )[]
  ...
  > = combineLatest([
    this.columnBodyCellTemplatesFromDirectivesChanges$,
    this.columnsTemplatesFromColumnsDirectives$,
  ]).pipe(
    map(([columnBodyCellTemplates, columnBodyCellTemplatesFromDirectives]) => [
      ...columnBodyCellTemplates,
      ...columnBodyCellTemplatesFromDirectives,
    ]),
    shareReplay({ bufferSize: 1, refCount: true })
  );
  ...
  private onColumnsConfigurationChange$ = this.columnsConfigurator.pipe(
    switchMap(
      (columnsConfigurator) =>
        columnsConfigurator?.onColumnsConfigurationChange ?? EMPTY
    ),
    share()
  );
  ...
  @ContentChildren(ImperiaTableV2ColumnDirective<TItem>)
  set columnsDirectivesSetter(
    v: QueryList<ImperiaTableV2ColumnDirective<TItem>>
  ) {
    this.columnsDirectives.next(v);
  }
  private columnsDirectives = new ReplaySubject<
    QueryList<ImperiaTableV2ColumnDirective<TItem>>
  >(1);
  private columnsFromDirectivesChanges$: Observable<
    Observable<ImperiaTableColumn<TItem>>[]
    ...
    switchMap((columnsDirectives) =>
      columnsDirectives.changes.pipe(
        ...
        map(() =>
          columnsDirectives
            .toArray()
            .map((colDirective) => colDirective.column$)
        )
      )
    )
  );

  public columnsFromDirectives$ = this.columnsFromDirectivesChanges$.pipe(
    switchMap((columns$) => combineLatest(columns$))
  );
  ...
  public columnsConfigured$: Observable<ImperiaTableColumn<TItem>[]> =
    this.columns$.pipe(
      withLatestFrom(this.storageKey, this.columnsConfigurator),
      map(([columns, storageKey, columnsConfigurator]) => ({
        columns,
        columnsFromStorage:
          columnsConfigurator?.getColumnsConfigurationFromStorage(storageKey) ??
          [],
        columnsConfigurator,
      })),
      map(({ columns, columnsFromStorage, columnsConfigurator }) => ({
        columns:
          columnsFromStorage.length && columnsConfigurator
            ? columnsConfigurator.applyColumnsConfiguration(
                columns,
                columnsFromStorage
              )
            : columns,
  ...
  public hasSomeHeader$ = combineLatest([
    this.columns$,
    this.columnHeaderCellTemplatesChanges$,
  ]).pipe(
    map(
      ([columns, columnHeaderCellTemplates]) =>
        columns.some(({ header }) => !!header) ||
        columnHeaderCellTemplates.length > 0
    )
  );
  ...
  //#region BODY ROW DETAIL COMPONENT
  @ContentChild(ImperiaTableV2RowDetailDirective)
  set imperiaTableV2RowDetailSetter(
    v: ImperiaTableV2RowDetailDirective<TItem> | undefined
  ) {
    this.imperiaTableV2RowDetail.next(v);
  }
  public imperiaTableV2RowDetail = new ReplaySubject<
    ImperiaTableV2RowDetailDirective<TItem> | undefined
  >(1);
  public hasRowDetail$ = this.imperiaTableV2RowDetail.pipe(
    map((imperiaTableV2RowDetail) => !!imperiaTableV2RowDetail)
  );
  //#endregion BODY ROW DETAIL COMPONENT
  ...
  private isScrollAtBottom({
    ...
    return offsetHeight + scrollTop >= scrollHeight * 0.9;
  }
  ...
  public orderedColumns$ = combineLatest([
    merge(this.columnsConfigured$, this.onColumnsConfigurationChange$).pipe(
      map((columns) =>
        columns.filter(
  ...
  private completeColumnsGroups(
    columnsGroups: ImperiaTableColumnsGroup<TItem>[],
    columns: ImperiaTableColumn<TItem>[]
  ) {
    return columns.reduce<ImperiaTableColumnsGroup<TItem>[]>(
      (completeColumnsGroups, col) => {
        const colGroup = columnsGroups.find(({ columns }) =>
          columns.includes(col)
        );
        if (colGroup) {
          if (completeColumnsGroups.length == 0) return [colGroup];
          if (completeColumnsGroups.includes(colGroup)) {
        ...
        if (!colGroup) {
          ...
          const lastPositionedColumnsGroupIndex = completeColumnsGroups.reduce(
            (lastPositionedIndex, colGroup, index) =>
              columnsGroups.includes(colGroup) ? index : lastPositionedIndex,
            -1
          );
          if (lastPositionedColumnsGroupIndex == -1) {
            completeColumnsGroups[completeColumnsGroups.length - 1].addColumn(
              col
            );
            return completeColumnsGroups;
          }
          const lastPlaceholderColumnsGroupIndex = completeColumnsGroups
            ...
            .findIndex((colGroup) => !columnsGroups.includes(colGroup));
          ...
          completeColumnsGroups[
            lastPositionedColumnsGroupIndex + lastPlaceholderColumnsGroupIndex
          ].addColumn(col);
        }
        return completeColumnsGroups;
      },
  ...
  private getFrozenColumnsGroupsAt(
    position: 'left' | 'right',
    allColumnsGroups: ImperiaTableColumnsGroup<TItem>[],
    positionedColumns: ImperiaTableColumn<TItem>[]
  ) {
    return this.completeColumnsGroups(
      allColumnsGroups.filter(
      ...
      {
        columnsGroups: [] as ImperiaTableColumnsGroup<TItem>[],
  ...
  private getUnfrozenColumnsGroups(
    ...
    return this.completeColumnsGroups(
      columnsGroups.filter(({ frozen }) => !frozen),
      unfrozenColumns
    );
  }
  ...
  public focusedColumnField$ = this.columnFieldToFocus.pipe(
    withValueFrom(
      this.orderedColumns$.pipe(
        filter(({ unfrozenColumns }) => unfrozenColumns.length > 0)
      ),
      this.viewport$
    ),
    ...
    tap(({ columnFieldToFocus, columnToFocusLeftOffset, viewport }) => {
      ...
      viewport.scrollable
        .elementScrolled()
        .pipe(take(1))
        .subscribe(() => clearInterval(interval));
    }),
  ...
  private getFirstVisibleColumnsGroupIndex(
    unfrozenColumnsGroups: ImperiaTableColumnsGroup<TItem>[],
  ...
  private markColumnsAsLastInGroup(
    unfrozenColumnsGroupsMatrix: ImperiaTableColumnsGroup<TItem>[][]
  ) {
  ...
  public async clickCell(
    ...
    if (
      editMode &&
      !row.cells[col.field].dataInfo.readonly &&
      !col.dataInfo.readonly &&
      row.editable
    ) {
    ...
    //#region TODO: DELETE THIS REGION WHEN onCellClickEmitter IS UNUSED
    const currentSelection = await firstValueFrom(this.rowSelection$, {
      defaultValue: [],
    });
    const isSelectedFn = await firstValueFrom(this.isRowSelectedFn$);
    const footerRows = await firstValueFrom(this.footerRows$, {
      defaultValue: [],
    });
    ...
    this.onCellClickEmitter.emit({
      ...
      isSelected: isSelectedFn(row, currentSelection),
      cellElement: element,
      updateCell: (data: Partial<TItem>) => {
        Object.assign(row.data, data);
        //Se actualiza el valor de la celda
        row.cells[col.field].value = data[col.field as keyof TItem] ?? null;
  ...
  /**
   * @deprecated Use `<imperia-table-v2-clicks (click)="onCellClick($event)">` instead
   * @see ImperiaTableV2ClicksDirective
   * @example
   * <imperia-table-v2>
   *  <imperia-table-v2-clicks (click)="onCellClick($event)">
   *  </imperia-table-v2-clicks>
   * </imperia-table-v2>
   */
  ...
  public headerCellFilterIconsTemplate$ = new ReplaySubject<
    TemplateRef<ImperiaTableHeaderCellIconsTemplateDirective<TItem>> | undefined
  >(1);
  public hasImperiaTableFilterV2$ = this.imperiaTableFilterV2Component.pipe(
    map((imperiaTableFilterV2Component) => !!imperiaTableFilterV2Component),
    shareReplay({ bufferSize: 1, refCount: true })
  );
  private onFilterChanges$: Observable<
    ImperiaTableFilterValue<TItem, string>[]
  > = this.imperiaTableFilterV2Component.pipe(
    distinctUntilChanged((prev, curr) => !!prev == !!curr),
    switchMap((imperiaTableFilterV2Component) =>
      !!imperiaTableFilterV2Component
        ? imperiaTableFilterV2Component.lastAppliedFilters$.pipe(
            map((filters) =>
              filters.map(({ Column }) => {
        ...
        : this.storage$.pipe(map(({ Filters }) => Filters))
    ),
    switchMap((Filters) =>
      forkJoin({
        Search: this.onSearchChanges$.pipe(take(1)),
        Sort: this.onSortChanges$.pipe(take(1)),
      }).pipe(
        map(({ Search, Sort }) => ({
          Filters,
          Search,
    ...
    withLatestFrom(this.pageSize),
    map(
      ([{ Filters, Search, Sort }, Size]) =>
        new ImperiaTableFilterSortScrollEvent({
          Pagination: { Size },
          Search,
          Order: Sort,
          Filters,
    ...
    tap((event) => this.onFilterChanges.emit(event)),
    map(({ Filters }) => Filters),
    shareReplay({ bufferSize: 1, refCount: true })
  );
  public toggleFilters = new Subject<void>();
  public filtersAreVisible$: Observable<boolean> = merge(
    ...
    merge(
      ...
      this.imperiaTableFilterV2Component.pipe(
        ...
        switchMap((imperiaTableFilterV2Component) =>
          !!imperiaTableFilterV2Component
            ? imperiaTableFilterV2Component.toggleOpened
            : EMPTY
        )
      )
    ).pipe(
      switchMap(() =>
        this.filtersAreVisible$.pipe(
          take(1),
          map((visible) => !visible)
        )
      )
  ...
  //#region SORTING
  @Input('allowSort') allowSort: boolean | null = null;
  public get sortAllowed() {
    if (this.allowSort == false) return false;
    if (this.onSortChanges.observed) return true;
    return false;
  }
  public onSortASC = new Subject<{
    col: ImperiaTableColumn<TItem>;
    close: () => void;
  }>();
  public onSortDESC = new Subject<{
    col: ImperiaTableColumn<TItem>;
    close: () => void;
  }>();
  public onSortClear = new Subject<{
    col: ImperiaTableColumn<TItem>;
    close: () => void;
  }>();
  private onSortChanges$: Observable<ImperiaTableSortValue<TItem>> = merge(
    this.onSortASC.pipe(map((event) => ({ ...event, sort: Sort.ASC }))),
    this.onSortDESC.pipe(map((event) => ({ ...event, sort: Sort.DESC }))),
    this.onSortClear.pipe(map((event) => ({ ...event, sort: Sort.NONE })))
  )
    .pipe(
      withLatestFrom(this.columnsConfigured$),
      tap(([event, columns]) => {
        columns.forEach((col) => col.sort(Sort.NONE));
        event.col.sort(event.sort);
        event.close();
      })
    )
    .pipe(
      map(
        ([{ col, sort }]) =>
          new ImperiaTableSortValue<TItem>(
            sort == Sort.NONE ? '' : col.filterName,
            sort
          )
      ),
      withLatestFrom(this.storageKey),
      tap(([Order, storageKey]) => setSortToStorage(storageKey, Order)),
      map(([Order]) => Order)
    )
    .pipe(
      switchMap((Order) =>
        this.onSearchChanges$.pipe(
          take(1),
          map((Search) => ({
            Order,
            Search,
      ...
      withLatestFrom(this.onFilterChanges$, this.pageSize),
      tap(([{ Search, Order }, Filters, Size]) =>
        this.onSortChanges.emit(
          new ImperiaTableFilterSortScrollEvent({
            Pagination: { Size },
            Search,
            Order,
      ...
      mergeWith(
        this.storage$.pipe(
          map(({ Order }) => Order),
          filter((Order) => !!Order.Column && Order.Sort != Sort.NONE)
        )
      ),
      startWith(new ImperiaTableSortValue<TItem>()),
      shareReplay({
        bufferSize: 1,
        refCount: true,
      })
  ...
  //#region SEARCH BAR VALUE
  @Input('search') set searchSetter(v: string | null) {
    if (v == null) return;
    this.onSearch.next(v);
  }
  public onSearch = new ReplaySubject<string>(1);
  public clearSearchBar = new Subject<HTMLInputElement>();
  public onSearchChanges$: Observable<string> = this.onSearch.pipe(
    debounceTime(500),
    distinctUntilChanged(),
    mergeWith(
      this.clearSearchBar.pipe(
        tap((input) => input.focus()),
        map(() => '')
      )
    ),
    withLatestFrom(this.onFilterChanges$, this.onSortChanges$, this.pageSize),
    tap(([Search, Filters, Order, Size]) =>
      this.onSearchChanges.emit(
        new ImperiaTableFilterSortScrollEvent({
          Pagination: { Size },
          Search,
          Order,
  ...
  private searchBarHasValue$ = this.onSearchChanges$.pipe(
    map((search) => search != null && search != ''),
    startWith(false),
    shareReplay({ bufferSize: 1, refCount: true })
  );
  ...
  //#region SEARCH BAR FOCUS HANDLING
  public onSearchBarFocusChange = new Subject<string | null>();
  public isSearchBarFocused$ = this.onSearchBarFocusChange.pipe(
    map((focus) => focus != null),
    startWith(false),
    shareReplay({ bufferSize: 1, refCount: true })
  );
  //#endregion SEARCH BAR FOCUS HANDLING

  //#region SEARCH BAR VISIBILITY
  @Input('allowSearch') allowSearch: boolean | null = null;
  public get searchAllowed() {
    if (this.allowSearch == false) return false;
    if (this.onSearchChanges.observed) return true;
    return false;
  }
  public openSearchBar = new Subject<HTMLInputElement>();
  public searchBarIsVisible$ = combineLatest([
    this.searchBarHasValue$,
    this.isSearchBarFocused$,
  ]).pipe(
    map(([hasValue, isFocused]) => hasValue || isFocused),
    mergeWith(
      this.openSearchBar.pipe(
        tap((input) => input.focus()),
        map(() => true)
      )
    ),
  ...
  public clearButtonIsVisible$ = merge(
    this.searchBarIsVisible$.pipe(
      filter((visible) => visible),
      delay(200)
    ),
    this.searchBarIsVisible$.pipe(filter((visible) => !visible))
  ).pipe(shareReplay({ bufferSize: 1, refCount: true }));
  //#endregion SEARCH BAR VISIBILITY

  //#region PAGINATION
  @Output() onPageChanges = new EventEmitter<
    ImperiaTableFilterSortScrollEvent<TItem>
  >();
  public onPageChanges$: Observable<ImperiaTableScrollValue> =
    this.isScrollAtBottom$
      .pipe(
        skip(1),
        filter(() => this.onPageChanges.observed),
        exhaustMap(() =>
          this.loading$.pipe(
            take(1),
            map(({ state }) => state)
        ...
        withLatestFrom(
          ...
          this.pageSize,
          defer(() =>
            this.onPageChanges$.pipe(startWith({ Page: 1, Size: 100 }))
          )
        ),
        filter(([_, items, pageSize, prevPagination]) => {
          return (
            (pageSize * prevPagination.Page) % items.length === 0 &&
            pageSize * prevPagination.Page === items.length
          );
        }),
        map(([_, items, pageSize]) => ({
      ...
      .pipe(
        ...
        tap(([Pagination, Filters, Order, Search]) =>
  ...
  //#region LOADING
  @Input('loading') set loadingSetter(v: ImperiaTableLoading<boolean> | null) {
    if (v == null) return;
    this.loading.next(v);
  }
  private loading = new ReplaySubject<ImperiaTableLoading>(1);
  public loading$: Observable<ImperiaTableLoading> = merge(
    this.loading,
    this.onFilterChanges$.pipe(
      skip(1),
      map(
    ...
    /* iif(
      () => this.onDelete.observed,
      this.onDeleteChanges$.pipe(
        map(
          () =>
            new ImperiaTableLoading({
              state: true,
    ...
    /* ,
    this.rows$.pipe(map(() => new ImperiaTableLoading(false))) */
  ).pipe(
    startWith(
      new ImperiaTableLoading({ state: true, at: 'top', from: 'startWith' })
    ),
    shareReplay({ bufferSize: 1, refCount: true })
  );
  //#endregion LOADING
  ...
  public filterByValue(
    ...
    imperiaTableFilterV2Component.addFilter.next(
